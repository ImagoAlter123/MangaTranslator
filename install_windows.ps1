...
Start-Transcript -Path (Join-Path $logDir 'installation.txt') -Append | Out-Null
try {
    ...
    & $venvPython (Join-Path $appRoot 'repair_pip.py')
    ...
    & $venvPython (Join-Path $appRoot 'download_models.py')
    if ($LASTEXITCODE -ne 0) { throw 'Failed to download or verify models. Run again to resume.' }
    Write-Host 'Ready. Use RUN.cmd. CC Wild Words Roman is already selected.'
} catch {
