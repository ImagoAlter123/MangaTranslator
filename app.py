...
class Window(QMainWindow):
    def __init__(self):
        super().__init__(); self.setWindowTitle('Manga Translator • edição local'); self.resize(1320,900)
        ...
        self.button(tools,'Abrir PDF / várias imagens',self.open)
        self.button(tools,'Abrir projeto',self.open_saved)
        self.button(tools,'Salvar projeto',self.save)
        self.button(tools,'Exportar PDF',self.export_pdf)
        self.button(tools,'Exportar página PNG',self.export_png)
        ...
        self.button(ll,'Sugerir balões desta página',self.detect)
        self.button(ll,'Fundo — experimental',self.start_background)
        self.button(ll,'Restaurar fundo original',self.reset_background)
        self.list=QListWidget(); self.list.currentRowChanged.connect(self.select); ll.addWidget(self.list)
        self.button(ll,'Excluir seleção',self.remove)
        self.button(ll,'Desfazer última alteração (Ctrl+Z)',self.undo)
        self.undo_shortcut=QShortcut(QKeySequence('Ctrl+Z'),self)
        self.undo_shortcut.setContext(Qt.WindowShortcut)
        self.undo_shortcut.activated.connect(self.undo)
        info=QLabel('Sugestões não apagam a página.\nRevise e marque “Aplicar tradução”.\n\nAzul: área para apagar\nVerde: área da tradução\n\nCtrl + roda: zoom\nBarras de rolagem: navegar'); info.setWordWrap(True); ll.addWidget(info)
        ...
        rr.addWidget(QLabel('Ao arrastar sobre a página:'))
        self.mode=QComboBox(); self.mode.addItems(['Criar balão manual','Ajustar área apagada','Ajustar área da tradução','Selecionar fundo experimental']); rr.addWidget(self.mode)
        self.original_view=QCheckBox('Mostrar página original'); self.original_view.toggled.connect(self.draw); rr.addWidget(self.original_view)
        self.button(rr,'Ajustar página à janela',self.fit)
        rr.addWidget(QLabel('Idioma original → inglês'))
        self.language=QComboBox(); self.language.addItems(['Japonês','Chinês']); rr.addWidget(self.language)
        from translator_hy import MODEL
        model_label=QLabel('Tradutor: Hy-MT2-7B Q8 • local\n'+('Modelo baixado' if MODEL.exists() else 'Execute INSTALAR_TUDO.cmd para baixar'))
        model_label.setWordWrap(True);rr.addWidget(model_label)
        rr.addWidget(QLabel('Glossário opcional (nomes / tratamentos)'))
        self.glossary=QLineEdit();self.glossary.setPlaceholderText('Ex.: 博士 = Doctor');rr.addWidget(self.glossary)
        self.vertical=QCheckBox('Texto original vertical'); self.vertical.setChecked(True); rr.addWidget(self.vertical)
        self.column_order=QComboBox();self.column_order.addItems(['Colunas: direita → esquerda','Colunas: esquerda → direita']);rr.addWidget(self.column_order)
        self.vertical.toggled.connect(self.column_order.setEnabled)
        rr.addWidget(QLabel('Original • editável'))
        self.source=QTextEdit(); self.source.setMaximumHeight(90); self.source.textChanged.connect(self.edit); rr.addWidget(self.source)
        self.button(rr,'Ler original com IA local',self.ocr)
        rr.addWidget(QLabel('Tradução em inglês • editável'))
        self.target=QTextEdit(); self.target.setMaximumHeight(90); self.target.textChanged.connect(self.edit); rr.addWidget(self.target)
        self.button(rr,'Traduzir original para inglês',self.translate)
        self.button(rr,'Ler e traduzir todos os balões',self.batch)
        self.button(rr,'Retraduzir toda a página com Hy-MT2',lambda:self.batch(True))
        self.font_label=QLabel('Fonte: CC Wild Words Roman (padrão)' if self.font else 'Fonte provisória: Arial'); self.font_label.setWordWrap(True); rr.addWidget(self.font_label)
        self.button(rr,'Carregar CC Wild Words (.ttf / .otf)',self.choose_font)
        fontrow=QHBoxLayout(); rr.addLayout(fontrow); fontrow.addWidget(QLabel('Tamanho em pixels:'))
        ...
        self.auto=QCheckBox('Fonte automática: preencher a área'); self.auto.setChecked(True); self.auto.toggled.connect(self.edit); rr.addWidget(self.auto)
        rr.addWidget(QLabel('Cor da fonte deste balão:'))
        self.text_color=QComboBox();self.text_color.addItems(['Preto','Branco']);self.text_color.currentIndexChanged.connect(self.edit);rr.addWidget(self.text_color)
        self.transparent=QCheckBox('Texto sobre arte (sem caixa branca)');self.transparent.toggled.connect(self.edit);rr.addWidget(self.transparent)
        outline_row=QHBoxLayout();rr.addLayout(outline_row);outline_row.addWidget(QLabel('Contorno contrastante (px):'))
        self.outline=QSpinBox();self.outline.setRange(0,12);self.outline.valueChanged.connect(self.edit);outline_row.addWidget(self.outline)
        self.enabled=QCheckBox('Aplicar tradução deste balão'); self.enabled.toggled.connect(self.edit); rr.addWidget(self.enabled)
        self.button(rr,'Aplicar todos os textos desta página',self.apply_all)
        rr.addWidget(QLabel('Todas as fontes desta página (tamanho manual):'))
        sizes=QHBoxLayout();rr.addLayout(sizes)
        self.button(sizes,'Diminuir 2 px',lambda:self.resize_all(-2))
        self.button(sizes,'Aumentar 2 px',lambda:self.resize_all(2))
        self.hint=QLabel('Abra um arquivo para começar.'); self.hint.setWordWrap(True); rr.addWidget(self.hint); rr.addStretch()
        split.setSizes([205,740,335]); self.statusBar().showMessage('Local • sem envio de páginas • inglês como destino')
    ...
    def discard(self):
        return not self.dirty or QMessageBox.question(self,'Alterações não salvas','Descartar alterações não salvas?',QMessageBox.Yes|QMessageBox.No,QMessageBox.No)==QMessageBox.Yes
    def install_pages(self,pages,font=''):
        ...
        self.font_label.setText(Path(self.font).name if self.font else 'Fonte provisória: Arial')
        self.page_combo.blockSignals(True); self.page_combo.clear(); self.page_combo.addItems([f'Página {i+1} de {len(pages)}' for i in range(len(pages))]); self.page_combo.blockSignals(False)
        self.index=0; self.refresh(); self.fit()
    def open(self):
        if not self.discard(): return
        paths,_=QFileDialog.getOpenFileNames(self,'Selecione um PDF ou várias imagens — use Ctrl+A, Ctrl ou Shift','','PDF e imagens (*.pdf *.png *.jpg *.jpeg *.webp)')
        if paths: self.run(lambda:load_multiple(paths),self.install_pages,f'Abrindo {len(paths)} arquivo(s) em ordem numérica…')
    def open_saved(self):
        if not self.discard(): return
        path,_=QFileDialog.getOpenFileName(self,'Abrir projeto','','Projeto (*.manga)')
        if path: self.run(lambda:load_project(path),lambda x:self.install_pages(*x),'Abrindo projeto…')
    def save(self):
        if not self.pages: return
        path,_=QFileDialog.getSaveFileName(self,'Salvar projeto','','Projeto (*.manga)')
        if path:
            try: save_project(path,self.pages,self.font); self.dirty=False; self.statusBar().showMessage('Projeto salvo, incluindo páginas originais.')
    ...
    def refresh(self,selected=0):
        ...
        if self.page(): self.list.addItems([f'Balão {i+1}'+(' • aplicado' if r.enabled else ' • revisar') for i,r in enumerate(self.page().regions)])
    ...
    def edit(self,*_):
        ...
        if item: item.setText(f'Balão {self.list.currentRow()+1}'+(' • aplicado' if r.enabled else ' • revisar'))
        self.draw()
    def apply_all(self):
        p=self.page()
        if not p:return
        ...
        self.statusBar().showMessage(f'{len(regions)} texto(s) aplicado(s) nesta página. Textos vazios foram ignorados. Use Desfazer para reverter.')
    def resize_all(self,delta):
        ...
        self.refresh(self.list.currentRow())
        self.statusBar().showMessage('Fontes ajustadas nesta página; tamanhos agora são manuais. Use Desfazer para reverter.')
    def draw(self,*_):
        p=self.page()
        if not p: return
        ...
        self.hint.setText('\n'.join(warnings) if warnings else 'Arraste para definir a área escolhida. Revise o texto antes de aplicar.')
    ...
    def start_background(self):
        if not self.page():self.error('Abra uma página antes de usar a ferramenta de fundo.');return
        self.mode.setCurrentIndex(3)
        self.statusBar().showMessage('Fundo experimental: arraste um retângulo ao redor do texto, incluindo um pouco do fundo limpo.')
    def edit_background(self,box):
        ...
        if dialog.exec()==BackgroundEditor.DialogCode.Accepted:
            ...
            self.statusBar().showMessage('Fundo aplicado. Use Desfazer para voltar; para traduzir sobre a arte, marque “Texto sobre arte”.')
    def reset_background(self):
        if self.page() and self.page().patches:
            self.checkpoint();self.page().patches=[];self.draw()
            self.statusBar().showMessage('Reparos do fundo desta página removidos. A ação pode ser desfeita.')
    def remove(self):
    ...
    def detect(self):
        ...
        def done(regions):
            ...
            self.refresh(); self.statusBar().showMessage(f'{len(regions)} sugestão(ões). Confira: a detecção pode confundir áreas brancas da arte com balões.')
        self.run(lambda:detect_bubbles(image),done,'Procurando balões brancos…')
    def choose_font(self):
        path,_=QFileDialog.getOpenFileName(self,'Selecione sua fonte CC Wild Words','','Fontes (*.ttf *.otf)')
        if path:
            try:
    ...
    def ocr(self):
        r=self.region()
        if not r:return
        image=self.page().image.crop(r.erase); language=self.language.currentText(); vertical=self.vertical.isChecked();rtl=self.column_order.currentIndex()==0
        self.run(lambda:self.ai.read(image,language,vertical,rtl),lambda text:self.source.setPlainText(text),'Lendo texto e comparando tratamento automático de fundo. Primeiro uso pode baixar o OCR…')
    def translate(self):
        if not self.region(): return
        source=self.source.toPlainText(); language=self.language.currentText();glossary=self.glossary.text()
        context=self.neighbor_context(self.page().regions,self.list.currentRow())
        self.run(lambda:self.ai.translate(source,language,context,glossary),lambda text:self.target.setPlainText(text),'Traduzindo com Hy-MT2-7B no processador. Pode demorar na VM…')
    @staticmethod
    ...
    def batch(self,replace=False):
        ...
        p=self.page(); language=self.language.currentText(); vertical=self.vertical.isChecked();glossary=self.glossary.text();rtl=self.column_order.currentIndex()==0
        ...
        def job():
            ...
            for i,r in enumerate(todo):
                if replace or not r.translation.strip():
                    ...
                    r.enabled=False
            return todo
        def done(regions): self.checkpoint(); p.regions=regions; self.refresh(); self.statusBar().showMessage('Traduções prontas para revisão. Aplique cada balão aprovado ou use Aplicar todos os textos desta página.')
        self.run(job,done,'Lendo e traduzindo os balões desta página…')
    ...
    def finished(self):
        ...
        if self.statusBar().currentMessage()==self.busy_message:self.statusBar().showMessage('Operação encerrada. Revise o resultado antes de aplicar.')
    def error(self,message): QMessageBox.warning(self,'Não foi possível concluir',message)
    def export_images(self,indices=None):
        ...
        if not self.font and any(r.enabled and r.translation.strip() for i in selected_pages for r in self.pages[i].regions):
            self.error('Carregue o arquivo CC Wild Words antes de exportar. A visualização usa Arial provisoriamente.');return None
        images=[]
        for i in selected_pages:
            p=self.pages[i]
            im,warnings=render_page(p,self.font,strict=True)
            if warnings:self.error(f'Página {i+1}:\n'+'\n'.join(warnings));return None
            images.append(im)
        return images
    def export_pdf(self):
        try:
            ...
            with __import__('pypdfium2').PdfDocument.new() as out:
                ...
                out.save(path)
            self.statusBar().showMessage('PDF exportado: '+path)
        except Exception as e:self.error(str(e))
    def export_png(self):
        try:
            images=self.export_images([self.index])
            if images:
                path,_=QFileDialog.getSaveFileName(self,'Exportar página','','PNG (*.png)')
                if path:images[0].save(path,dpi=(self.page().dpi,self.page().dpi))
        except Exception as e:self.error(str(e))
    def closeEvent(self,event):
        if self.worker:
            self.error('Aguarde a operação atual terminar antes de fechar.');event.ignore()
